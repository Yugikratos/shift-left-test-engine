...
from config.settings import PII_NAME_PATTERNS, PII_ADDRESS_PATTERNS, PII_PHONE_PATTERNS, ENTERPRISE_MODE, BASE_DIR
    ...
    def execute(self, context: dict) -> AgentResult:
        ...
        if ENTERPRISE_MODE:
            ...
            with open(script_path, "w") as f:
                f.write("/* Ab Initio Transform Script Generated by Google Gemini */\n")
                f.write(f"/* Target Tables: {', '.join(extracted_data.keys() if extracted_data else [])} */\n")
                f.write("out :: reformat(in) = begin\n")
                f.write("  out.* :: in.*;\n")
                f.write("  // MASKING RULES WOULD BE INJECTED HERE\n")
                f.write("end;\n")
                
            from utils.remote_executor import RemoteExecutor
            executor = RemoteExecutor(host="rhel-etl-prod.internal", user="ab_svc")
            executor.connect()
